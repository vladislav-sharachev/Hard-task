...
public class CalcTest {
   ...
   /* @Test (dependsOnMethods = "SetInMemory")
    public void getInMemory() { //запуск после метода setInMemory
        double firstNumber = 100;
        double result = calc.getMemoryResult();
        Assert.assertEquals(result, 100, delta, "Ошибка во время чтения памяти");
    }
    ...
    @AfterClass (dependsOnMethods = "clearMemory1") //такой возможности здесь нет он выполняется последовательно
    public void clearMemory2() {
        System.out.println("clearMemory");
    }

    @AfterClass // @AfterMethod этот метод запускается после каждого метода и получается что к методу getInMemory() уже ничего не остается
    public void clearMemory1() {
        calc.clearMemory();
    } */
}
